...
function mensajeAlerta(cMensaje, fnCallback) {
	...
	if (typeof (fnCallback) == 'function') {
		$('#dialogoMensaje').on('hide.bs.modal', function(e) {
			fnCallback();
			$('#dialogoMensaje').unbind('hide.bs.modal');
		});
	}
...
function fnGrabarParcial(valor) {
	...
	$.ajax({
		...
		data : {
			'prm_dataSource' : 'cotizarte',
			'prm_funcion' : 'paCotizarteAGV.operLicitacionUpdParcial',
			'prm_PLICITACION' : $('#acc_PLICITACION').val(),
			'prm_CEMAIL' : $('#acc_CEMAIL').val(),
			'prm_CRAZONSOCIAL' : $('#acc_CRAZONSOCIAL').val().toUpperCase(),
			'prm_NCUIT' : toNumberEmpty($('#acc_NCUIT').val()),
			'prm_FPROVINCIA' : $('#acc_FPROVINCIA').val(),
			'prm_CDOMICILIO' : $('#acc_CDOMICILIO').val(),
			'prm_FPOSTAL' : $('#acc_FPOSTAL').val(),
			'prm_NPOSTAL' : toNumberEmpty($('#acc_NPOSTAL').val()),
			'prm_CCONTACTO' : $('#acc_CCONTACTO').val(),
			'prm_NCAPITAS' : toNumberEmpty($('#acc_NCAPITAS').val()),
			'prm_MNOMINA' : toNumberEmpty($('#acc_MNOMINA').val()),
			'prm_FARTACTUAL' : $('#acc_FARTACTUAL').val(),
			'prm_FESTADOWKF' : $('#acc_FESTADOWKF').val(),
			'prm_NFIJO_ARTACTUAL' : toNumberEmpty($('#acc_NFIJO_ARTACTUAL').val()),
			'prm_MALICUOTA_ARTACTUAL' : toNumberEmpty($('#acc_MALICUOTA_ARTACTUAL').val()),
			'prm_BMAIL' : '0'
		},
		type : 'post',
		success : function(data) {
			...
			if (info.NESTADO > 0)
				$('#acc_PLICITACION').val(info.NESTADO);
		},
		error : function(xhr, ajaxOptions, thrownError) {
		}
	});
...
function fnGrabar() {
	...
	var fld_CDOMICILIO = $('#acc_CDOMICILIO');
	...
	var fld_FARTACTUAL = $('#acc_FARTACTUAL');
	if (isNaN(fld_FARTACTUAL.val()))
		fld_FARTACTUAL.val('');
	else
		fld_FARTACTUAL.val(parseInt(fld_FARTACTUAL.val()));

	var fld_NFIJO_ARTACTUAL = $('#acc_NFIJO_ARTACTUAL');
	...
	var fld_MALICUOTA_ARTACTUAL = $('#acc_MALICUOTA_ARTACTUAL');
